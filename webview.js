/**
 * devRant integration plugin for Franz
 *
...
let feedScripts = {
	init : function() {
		if ($('.addcomment-btn').length) {
			...
			this.addPermalink(rant);
		} else if ($('.addrant-btn').length) {
			this.autoScroll();
			this.nrantHandler();
		}
	...
	/**
	 * Adds the permalink button to individual rants
	 *
	 * @since       1.3.0
	 * @param       {string} rant The ID of the rant we are viewing
	 * @return      {void}
	 */
	addPermalink : function(rant) {
	...
	nrantHandler : function() {
		let nrants = readCookie('fdr_setting_nrants');

		if (nrants === 'hide') {
			$('.rantlist-title-text').each( function() {
				if ($(this).is(":contains('!rant')")) {
					$(this).closest('.rant-comment-row-widget').css('display', 'none');
				}
			});
			
			$('.rantlist-tags a').each( function() {
				if ($(this).is(":contains('!rant')")) {
					$(this).closest('.rant-comment-row-widget').css('display', 'none');
				}
			});
			...
			$('.rantlist-title-text').each( function() {
				if ($(this).is(":contains('!rant')")) {
					$(this).closest('.rant-comment-row-widget').css('display', 'block');
				}
			});
...
let aboutScripts = {
	init : function() {
		// Right now, about only works on non-profile pages
		if (! $('.profile-bar').length || $('.notif-bar').length) {
			this.addIcon();
		} else {
	 ...
	 */
	addIcon : function() {
		$('.menu-icon').after('<div class="franz-icon"><a href="#" title="Franz Integration">' + franzIcon + '</a><span class="franz-update-notification" style="display: none">&bull;</span></div>');
		
		$('.franz-icon a').click( function(e) {
			e.preventDefault();
	 ...
	 *
	 * @since       1.3.0
	 * @param       {string} list The list to append about to
	 */
	renderDialog : function(list) {
		let nrants = readCookie('fdr_setting_nrants');
		
		// Fallbacks
		nrants = nrants === 'hide' ? 'hide' : 'default';
		
		// Setup the dialog in chunks for easier tweaking later
		let aboutWrapperBefore = '<div class="franz-about-box" style="display: none">';
		...
		let aboutInfoTitle = '<div class="franz-about-title">devRant Franz Integration</div>';
		let aboutInfoVersion = '<div class="franz-about-version"><span>Version ' + currentVer + '</span><a href="https://github.com/extendfranz/devrant/blob/' + currentVer + '/CHANGELOG.md" target="_blank">What\'s New</a><a href="https://www.devrant.io/rants/446630">Integration Rant</a></div>';
		let aboutInfoUpdate = '<div class="franz-about-update">' + preUpdate + '</div>';
		let aboutInfoWrapperAfter = '</div><div class="clearfix"></div>';
		
		let aboutInfoBox = aboutInfoWrapperBefore + aboutInfoTitle + aboutInfoVersion + aboutInfoUpdate + aboutInfoWrapperAfter;
		
		// Details box
		let aboutDetailsBox = '<div class="franz-about-details"><p>The devRant integration for Franz is designed by <a href="http://extendfranz.com" target="_blank">ExtendFranz</a> and is released under the MIT license.</p><p>Questions? Comments? <a href="https://github.com/extendfranz/devrant/issues" target="_blank">Let us know</a>!</p><p>Want to help? <a href="http://extendfranz.com/donate/" target="_blank">Make a donation</a> or <a href="https://github.com/extendfranz/devrant/issues" target="_blank">get involved</a>!</p></div>';
		
		// Settings box
		let aboutSettingsBoxBefore = '<div class="franz-about-settings"><hr />';
		let aboutSettingsBoxNRants = '<span>!rants:</span> <form action="">' + 
										'<input id="default" name="nrants" value="default" type="radio"' + (nrants === 'default' ? 'checked="checked"' : '') + '><label for="default">Do Nothing</label>&nbsp;' +
										'<input id="hide" name="nrants" value="hide" type="radio"' + (nrants === 'hide' ? 'checked="checked"' : '') + '><label for="hide">Hide</label>&nbsp;' +
										'</form>';
		let aboutSettingsBoxAfter = '<hr /></div>';
		
		let aboutSettingsBox = aboutSettingsBoxBefore + aboutSettingsBoxNRants + aboutSettingsBoxAfter;
		
		// Footer box
		let aboutFooterBox = '<div class="franz-about-footer">Franz and the Franz logo are copyright Stefan Malzner.<br />This plugin is not affiliated with or endorced by Franz in any way.</div>';
		
		let aboutWrapperAfter = '</div>';
		
		$(list).after(aboutWrapperBefore + aboutIconBox + aboutInfoBox + aboutDetailsBox + aboutSettingsBox + aboutFooterBox + aboutWrapperAfter);
		
		$('input[name="nrants"]').change( function() {
			createCookie('fdr_setting_nrants', $('input[name="nrants"]:checked').val(), 720);
		});
	},
	/**
	 * Renders the help link on my profile
	 *
	 * @since       1.3.0
	 */
...
function toggleAboutDialog() {
	...
	if ($('.rantlist').length) {
		list = $('.rantlist');
	} else if ($('.notif-list').length) {
		list = $('.notif-list');
	}
	
	if (! $('.franz-about-box').length) {
		aboutScripts.renderDialog(list);
	}
	...
	if ($(list).css('display') != 'none') {
		...
		$('.franz-about-box').fadeOut('fast', function () {
			// Hide the about box
			$(this).css('display', 'none');
			$('.franz-about-update').html(preUpdate);

			// Display the rant list
			$(list).fadeIn('fast').css('display', 'block');

			// Display relevant button
			$(button).fadeIn('fast').css('display', 'block');
			
			if (view === 'rantlist') {
				feedScripts.nrantHandler(true);
			}
		});
	}
}
...
function updateCheck() {
	let latestVer = readCookie('fdr_latest_version');

	if(latestVer === null) {
		$.get('https://raw.githubusercontent.com/extendfranz/devrant/master/package.json', function(data) {
			let packagedata = $.parseJSON(data);

			if (packagedata) {
				latestVer = packagedata.version;
				
				createCookie('fdr_latest_version', latestVer, 24);
				
				updateDisplay(latestVer);
			} else {
				eraseCookie('fdr_latest_version');
...
function updateDisplay(latestVer) {
	if(latestVer === 'error') {
		...
		$('.franz-update-notification').css('display', 'none');
		...
		$('.franz-update-notification').css('display', 'block');
	} else {
